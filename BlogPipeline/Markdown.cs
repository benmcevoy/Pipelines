...
namespace BlogPipeline
{
    ...
    public class Markdown
    {
        ...
        private string StripLinkDefinitions(string text)
        {
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(LinkEvaluator),
        ...
        {
            var linkID = match.Groups[1].Value.ToLower();
        ...
        private string HashHTMLBlocks(string text)
        {
            ...
            */
            ...
            var blockTags2 = "p|div|h[1-6]|blockquote|pre|table|dl|ol|ul|script|noscript|form|fieldset|iframe|math";
            ...
            */
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(HtmlEvaluator),
                                 RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace);
        ...
        private ArrayList TokenizeHTML(string text)
        {
            ...
            var nestedTags = string.Concat(RepeatString(@"(?:<[a-z\/!$](?:[^<>]|", depth),
                                           RepeatString(@")*>)", depth));
            var pattern = string.Concat(@"(?s:<!(?:--.*?--\s*)+>)|(?s:<\?.*?\?>)|", nestedTags);

            var mc = Regex.Matches(text, pattern, RegexOptions.IgnoreCase | RegexOptions.Multiline);

            foreach (Match m in mc)
            {
                var wholeTag = m.Value;
        ...
        private string EscapeSpecialChars(string text)
        {
            ...
            {
                var value = token.Second.ToString();
        ...
        private string DoAnchors(string text)
        {
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(AnchorReferenceEvaluator),
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(AnchorInlineEvaluator),
                                 RegexOptions.Singleline | RegexOptions.IgnorePatternWhitespace);
        ...
        private string AnchorReferenceEvaluator(Match match)
        {
            ...
            var linkID = match.Groups[3].Value.ToLower();
            string url = null;
            string res = null;
            string title = null;
        ...
        private string AnchorInlineEvaluator(Match match)
        {
            ...
            var isTargetBlank = match.Groups[7].Value == "+";
            string res = null;
            ...
            {
                title = title.Replace("\"", "&quot;").Replace("*", escapeTable["*"].ToString()).Replace("_",
                                                                                                        escapeTable["_"]
                                                                                                            .ToString());
                res += string.Format(" title=\"{0}\"", title);
            }
        ...
        private string DoImages(string text)
        {
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(ImageReferenceEvaluator),
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(ImageInlineEvaluator),
                                 RegexOptions.IgnorePatternWhitespace | RegexOptions.Singleline);
        ...
        private string DoHeaders(string text)
        {
            ...
            */
            ...
            text = Regex.Replace(text, @"^(.+)[ \t]*\n=+[ \t]*\n+", new MatchEvaluator(SetextHeader1Evaluator),
                                 RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace);
            text = Regex.Replace(text, @"^(.+)[ \t]*\n-+[ \t]*\n+", new MatchEvaluator(SetextHeader2Evaluator),
                                 RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace);
            ...
            */
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(AtxHeaderEvaluator),
                                 RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace);
        ...
        {
            var header = match.Groups[1].Value;
            return string.Concat("<h3>", RunSpanGamut(header), "</h3>\n\n");
        }

        private string AtxHeaderEvaluator(Match match)
        {
            var headerSig = match.Groups[1].Value;
            var headerText = match.Groups[2].Value;

            return string.Concat("<h", (headerSig.Length + 1), ">", RunSpanGamut(headerText), "</h", (headerSig.Length + 1), ">\n\n");
        }
        ...
        private string DoLists(string text)
        {
            ...
            {
                pattern = "^" + wholeList;
                text = Regex.Replace(text, pattern, new MatchEvaluator(ListEvaluator),
                                     RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace);
            }
            else
            {
        ...
        private string DoBlockQuotes(string text)
        {
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(BlockQuoteEvaluator),
                                 RegexOptions.IgnorePatternWhitespace | RegexOptions.Multiline);
            return text;
        }
        ...
        private string BlockQuoteEvaluator(Match match)
        {
            ...
            bq = Regex.Replace(bq, @"(\s*<pre>.+?</pre>)", new MatchEvaluator(BlockQuoteEvaluator2),
                               RegexOptions.IgnorePatternWhitespace | RegexOptions.Singleline);
        ...
        private string FormParagraphs(string text)
        {
            ...
            for (var i = 0; i < grafs.Length; i++)
            {
                var block = (string)htmlBlocks[grafs[i]];
        ...
        private string DoAutoLinks(string text)
        {
            ...
            text = Regex.Replace(text, pattern, new MatchEvaluator(EmailEvaluator),
        ...
        {
            var email = UnescapeSpecialChars(match.Groups[1].Value);
        ...
        {
            var c = Convert.ToChar(match.Groups[1].Value);
        ...
        {
            return Regex.Replace(block, @"^(\t|[ ]{1," + tabWidth + @"})", string.Empty, RegexOptions.Multiline);
        }
        ...
        private static string ComputeMD5(string text)
        {
            ...
            var hashedText = algo.ComputeHash(plainText);
            string res = null;

            foreach (var b in hashedText)
                res += b.ToString("x2");
        ...
        private readonly Hashtable htmlBlocks;
        private readonly Hashtable titles;
        private readonly Hashtable urls;
